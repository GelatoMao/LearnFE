...
fs.readFile(__dirname + '/demo.txt', (err, data) => {
  if (err) console.log(err, '文件读取失败')
  else console.log(data.toString()) // <Buffer 68 61 68 61 68 61 68 61 68 61> 读取的是16进制的编码 需要添加toString() 读取出来的东西都是Buffer 因为用户存储的不一定是纯文本
  fs.writeFile(__dirname + '/demo1.txt', data, err => {
